...
function PdfEditBlock(runtime, element) {
    ...
    $(element).find('.save-button').on('click', function(e) {
        ...
        var data = {
            ...
            allow_download: $(element).find('select[id=edit_allow_download]').val() === 'true',
            source_text: $(element).find('input[id=edit_source_text]').val(),
            source_url: $(element).find('input[id=edit_source_url]').val()
        };
    ...
    $(element).find('.setting-clear').on('click', function(e) {
        e.preventDefault();
